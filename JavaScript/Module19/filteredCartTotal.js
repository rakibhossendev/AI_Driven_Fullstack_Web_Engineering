...
const getExpensiveItemsTotal = (cart,miniPrice) => {
    let expensiveProductTotal = cart.filter((value)=> value.price >= miniPrice)
    .map((value)=> value.price * value.quantity)
    .reduce((accu,value)=> accu + value,0);

    return expensiveProductTotal
}

console.log(getExpensiveItemsTotal(products,20))