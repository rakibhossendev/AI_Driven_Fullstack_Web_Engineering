...
interface AppDataState{
    ...
    numbersDataset: NumberDataset;
}
const shoppingCart:AppDataState = {
  "shoppingCart": {
    "items": [
      {
        "id": "prod-101",
...
const authenticationCheck = (userObject: AppDataState): boolean =>{
	const {userSession: { isLoggedIn }} = userObject;
	return isLoggedIn
}

const userFilteredData = (userObject: AppDataState) =>{
	...
	const filteredEvenNumber:number[] = originalArray.filter((el => el%2==0));
	const totalPrice: number = items.reduce((accumulator,value) => accumulator + value.price,0);
